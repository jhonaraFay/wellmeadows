...
// Handle form submission for add_ward.php
if (basename($_SERVER['PHP_SELF']) === 'add_ward.php' && $_SERVER['REQUEST_METHOD'] === 'POST') {
...
// Handle form and fetch for edit_ward.php
if (basename($_SERVER['PHP_SELF']) === 'edit_ward.php') {
    if (!isset($_GET['id'])) {
        header("Location: index.php");
        exit;
    }
...
<head>
    <meta charset="UTF-8">
    <title><?php echo basename($_SERVER['PHP_SELF']) === 'add_ward.php' ? 'Add New Ward' : 'Edit Ward'; ?></title>
    ...
    <div class="row">
        ...
        <main class="col-md-10 ms-sm-auto px-md-4 content">
            <div class="mt-4 mb-4 d-flex justify-content-between align-items-center">
                <h2><?php echo basename($_SERVER['PHP_SELF']) === 'add_ward.php' ? 'Add New Ward' : 'Edit Ward'; ?></h2>
            </div>

            <form method="POST">
                <div class="mb-3">
                    <label for="ward_name" class="form-label">Ward Name</label>
                    <input type="text" name="ward_name" id="ward_name" class="form-control" value="<?php echo isset($ward['ward_name']) ? htmlspecialchars($ward['ward_name']) : ''; ?>" required>
                </div>
                <div class="mb-3">
                    <label for="location" class="form-label">Location</label>
                    <input type="text" name="location" id="location" class="form-control" value="<?php echo isset($ward['location']) ? htmlspecialchars($ward['location']) : ''; ?>" required>
                </div>
                <div class="mb-3">
                    <label for="total_beds" class="form-label">Total Beds</label>
                    <input type="number" name="total_beds" id="total_beds" class="form-control" value="<?php echo isset($ward['total_beds']) ? htmlspecialchars($ward['total_beds']) : ''; ?>" required>
                </div>
                <div class="mb-3">
                    <label for="telephone_extension" class="form-label">Telephone Extension</label>
                    <input type="text" name="telephone_extension" id="telephone_extension" class="form-control" value="<?php echo isset($ward['telephone_extension']) ? htmlspecialchars($ward['telephone_extension']) : ''; ?>" required>
                </div>
                <button type="submit" class="btn btn-custom-green"><?php echo basename($_SERVER['PHP_SELF']) === 'add_ward.php' ? 'Add Ward' : 'Update Ward'; ?></button>
            </form>
        </main>
    </div>
</div>
